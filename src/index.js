...
import "./styles.css"; // Make sure you have a styles.css in src
...
const initialUsers = [
  { id: "1", name: "Irakli Shengelia", userType: UserType.FARMER },
  { id: "2", name: "Irakli Shengelia", userType: UserType.OPERATOR },
];
...
const initialBookings = [
  {
    ...
    dateRange: { start: new Date(), end: nowPlusDays(2) },
    status: "Pending",
...
function FarmerDashboard({
  ...
  const totalArea = myBookings.reduce((sum, b) => sum + (b.field.size || 0), 0);

  return (
    <div style={{ padding: 16 }}>
      <h1>{t(currentLanguage, "farmerDashboard")}</h1>
      <button onClick={onLanguageToggle}>{t(currentLanguage, "language")}</button>
...
function AddFieldScreen({ currentLanguage, onAddField, user }) {
  ...
  const crops = ["Corn", "Wheat", "Barley", "Orchard", "Vegetables", "Sunflowers"];

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!fieldName || !fieldSize || !farmerMobile) {
      alert(t(currentLanguage, "fieldName") + " / " + t(currentLanguage, "fieldSize"));
      return;
    }
    const newField = {
...
// DeleteFieldScreen
function DeleteFieldScreen({ currentLanguage, fields, onDeleteField, bookings }) {
  const { id } = useParams();
  const navigate = useNavigate();
  const field = fields.find((f) => f.id === id);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {hasActiveBooking ? (
        <p style={{ color: "red" }}>Cannot delete this field—it has an active booking!</p>
      ) : (
        <>
...
function RequestSprayScreen({ currentLanguage, user, fields, onAddBooking }) {
  ...
  const handleSubmit = (e) => {
    ...
    const price = field.size * 50; // example formula
